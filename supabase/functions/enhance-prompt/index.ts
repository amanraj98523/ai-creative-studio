...
serve(async (req) => {
  ...
  try {
        ...
        tools: [
          {
            ...
            function: {
              ...
              parameters: {
                ...
                required: ["tone", "intent", "requirements", "enhancedPrompt", "reasoning"],
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) return jsonResponse({ error: "Rate limit reached. Please retry shortly." }, 429);
      if (response.status === 402) return jsonResponse({ error: "Lovable AI credits required. Add workspace usage credits and retry." }, 402);
      return jsonResponse({ error: `AI enhancement failed [${response.status}]: ${text}` }, 500);
    }

    const payload = await response.json();
    const toolCall = payload?.choices?.[0]?.message?.tool_calls?.[0]?.function?.arguments;
    if (!toolCall) {
      return jsonResponse({ error: "Invalid enhancement response from AI gateway." }, 500);
    }
    ...
    return jsonResponse({
      ...
      requirements: result.requirements.map((item) => String(item).slice(0, 80)).slice(0, 8),
